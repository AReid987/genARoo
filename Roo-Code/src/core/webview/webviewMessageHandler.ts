...
// Define API base URL (consider making this configurable later)
// const ROO_PERSISTENCE_API_BASE_URL = 'http://localhost:8000/api/v1'; // Commented out or removed
...
export const webviewMessageHandler = async (provider: ClineProvider, message: WebviewMessage) => {
    ...
    const getApiBaseUrl = () => {
        return vscode.workspace.getConfiguration('roo-cline').get<string>('apiBaseUrl') || 'http://localhost:8000/api/v1';
    };
	...
	switch (message.type) {
		...
		case "updateCustomMode":
			if (message.modeConfig) {
				// Existing local file update
				await provider.customModesManager.updateCustomMode(message.modeConfig.slug, message.modeConfig);

				// Prepare payload for the API, excluding client-side 'source'
				const { source, ...apiPayloadRest } = message.modeConfig;
				const payload = {
					slug: apiPayloadRest.slug,
					name: apiPayloadRest.name,
					description: apiPayloadRest.description,
					system_prompt: apiPayloadRest.systemPrompt,
					user_prompt: apiPayloadRest.userPrompt,
					tags: apiPayloadRest.tags,
					version: apiPayloadRest.version,
					provider_settings: apiPayloadRest.providerSettings,
				};

				const API_BASE_URL = getApiBaseUrl();

				try {
					// Try to PUT (update) first
					provider.log(`[API Integration] Attempting to update Roomode: ${payload.slug}`);
					await axios.put(`${API_BASE_URL}/roomodes/${payload.slug}`, payload);
					provider.log(`[API Integration] Successfully updated Roomode: ${payload.slug}`);
				} catch (error) {
					if (axios.isAxiosError(error) && error.response && error.response.status === 404) {
						// If 404, try to POST (create)
						provider.log(`[API Integration] Roomode ${payload.slug} not found for update, attempting to create.`);
						try {
							await axios.post(`${API_BASE_URL}/roomodes/`, payload);
							provider.log(`[API Integration] Successfully created Roomode: ${payload.slug}`);
						} catch (postError) {
							const postErrorMessage = postError instanceof Error ? postError.message : String(postError);
							provider.log(`[API Integration] Error creating Roomode ${payload.slug}: ${postErrorMessage}`);
							vscode.window.showErrorMessage(`Failed to create Roomode '${payload.slug}' in database: ${postErrorMessage}`);
						}
					} else {
						// Other error during PUT
						const putErrorMessage = error instanceof Error ? error.message : String(error);
						provider.log(`[API Integration] Error updating Roomode ${payload.slug}: ${putErrorMessage}`);
						vscode.window.showErrorMessage(`Failed to update Roomode '${payload.slug}' in database: ${putErrorMessage}`);
					}
				}

				// Update state after saving the mode (local and potentially remote)
				const customModes = await provider.customModesManager.getCustomModes();
				await updateGlobalState("customModes", customModes);
		...
		case "deleteCustomMode":
			if (message.slug) {
				...
				// API call to delete
				try {
					provider.log(`[API Integration] Attempting to delete Roomode: ${message.slug}`);
					await axios.delete(`${API_BASE_URL}/roomodes/${message.slug}`);
					provider.log(`[API Integration] Successfully deleted Roomode: ${message.slug} from database.`);
				} catch (error) {
					const deleteErrorMessage = error instanceof Error ? error.message : String(error);
					provider.log(`[API Integration] Error deleting Roomode ${message.slug} from database: ${deleteErrorMessage}`);
					// Optionally notify user, but local deletion has already occurred.
					// vscode.window.showErrorMessage(`Failed to delete Roomode '${message.slug}' from database: ${deleteErrorMessage}`);
				}
