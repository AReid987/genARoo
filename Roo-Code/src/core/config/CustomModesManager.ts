...
export class CustomModesManager {
	...
	public async getCustomModes(): Promise<ModeConfig[]> {
		...
		const roomodesPath = await this.getWorkspaceRoomodes();
		const projectModes = roomodesPath ? await this.loadModesFromFile(roomodesPath) : [];

		let localModes = await this.mergeCustomModes(projectModes, globalModes);

		// Fetch from API
		const API_BASE_URL = vscode.workspace.getConfiguration('roo-cline').get<string>('apiBaseUrl') || 'http://localhost:8000/api/v1';
		try {
			logger.info(`[API Integration] Fetching Roomodes from ${API_BASE_URL}/roomodes/`);
			const response = await axios.get(`${API_BASE_URL}/roomodes/`);
			logger.info(`[API Integration] Successfully fetched Roomodes from API.`);
			const apiModes: ModeConfig[] = response.data.map((mode: any) => ({
				...mode,
				systemPrompt: mode.system_prompt,
                userPrompt: mode.user_prompt,
                providerSettings: mode.provider_settings,
				source: 'api' as const // Mark as API source
			}));

			// Merge API modes with local modes (local takes precedence)
			const finalMergedModes = [...localModes];
            const localSlugs = new Set(localModes.map(m => m.slug));

            for (const apiMode of apiModes) {
                if (!localSlugs.has(apiMode.slug)) {
                    finalMergedModes.push(apiMode);
                }
            }
            localModes = finalMergedModes;

		} catch (error) {
			const errorMsg = error instanceof Error ? error.message : String(error);
			logger.error(`[API Integration] Error fetching Roomodes from API: ${errorMsg}`);
			// vscode.window.showErrorMessage(`Failed to fetch Roomodes from API: ${errorMsg}`); // Optional: notify user
		}

		await this.context.globalState.update("customModes", localModes);
		this.cachedModes = localModes;
		this.cachedAt = now;
